...
import { UploadCloud, FileText, Calendar, MapPin, Users, List, FileImage, Loader2, Plus, Clock, Info, Copy, Check, ArrowLeft, ExternalLink, Pencil, Save, Download, Mail } from 'lucide-react';
...
หมายเหตุ: ${editedDetails.notes}
  ...
  const downloadIcsFile = (details: MeetingDetails) => {
    ...
    if (agenda && agenda.length > 0) {
      description += `วาระการประชุม: ${agenda.map(a => `- ${a}`).join('\\n')}`;
    }
    ...
    link.setAttribute('download', `${topic || 'meeting'}.ics`);
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="pt-6 border-t border-gray-100 space-y-4">
                    <h4 className="text-center text-[11px] font-bold text-gray-500 uppercase tracking-widest">ลงบันทึกการเชิญประชุม</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <motion.a
                        ...
                        className="group flex items-center gap-3.5 p-3.5 bg-white border border-gray-100 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/20 transition-all duration-300 shadow-sm"
                      >
                        <div className="w-10 h-10 bg-white border border-gray-100 rounded-xl flex items-center justify-center shrink-0 shadow-sm group-hover:border-indigo-200">
                          <GoogleLogo />
                        </div>
                        <div className="text-left overflow-hidden">
                          <p className="text-[11px] font-bold text-gray-900 truncate">Google Calendar</p>
                          <p className="text-[9px] text-gray-400">ลงปฏิทินกูเกิล</p>
                        </div>
                        <ExternalLink className="w-3.5 h-3.5 text-gray-300 ml-auto shrink-0" />
                      </motion.a>

                      <motion.a
                        whileTap={{ scale: 0.98 }}
                        href={generateOutlookWebUrl(editedDetails!)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group flex items-center gap-3.5 p-3.5 bg-white border border-gray-100 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/20 transition-all duration-300 shadow-sm"
                      >
                        <div className="w-10 h-10 bg-[#0078d4] rounded-xl flex items-center justify-center shrink-0 shadow-sm">
                          <OutlookLogo />
                        </div>
                        <div className="text-left overflow-hidden">
                          <p className="text-[11px] font-bold text-gray-900 truncate">Outlook Web</p>
                          <p className="text-[9px] text-gray-400">email.pea.co.th</p>
                        </div>
                        <ExternalLink className="w-3.5 h-3.5 text-gray-300 ml-auto shrink-0" />
                      </motion.a>
                      ...
                      <motion.button
                        whileTap={{ scale: 0.98 }}
                        onClick={() => downloadIcsFile(editedDetails!)}
                        className="group flex items-center gap-3.5 p-3.5 bg-white border border-gray-100 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/20 transition-all duration-300 shadow-sm"
                      >
                        ...
                        <div className="text-left overflow-hidden">
                          <p className="text-[11px] font-bold text-gray-900 truncate">ไฟล์นัดหมาย (.ics)</p>
                          <p className="text-[9px] text-gray-400">สำหรับโปรแกรม Outlook</p>
                        </div>
                        <Download className="w-3.5 h-3.5 text-gray-300 ml-auto shrink-0" />
                      </motion.button>
                    </div>
                  </div>
                </div>
